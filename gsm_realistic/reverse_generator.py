...
from DependencyGraph import (
    RNG,
    AbstractParameter,
    AbstractParameterSpecial,
    DependencyGraph,
    InstanceParameter,
    ParameterNode,
)
from simple_dummy_namesone import dummyhierarchical_categorizations, dummysubcategories
from simple_dummy_namesthree import (
    dummyhierarchical_categorizationsthree,
    dummysubcategoriesthree,
)
from simple_dummy_namestwo import (
    dummyhierarchical_categorizationstwo,
    dummysubcategoriestwo,
)
from simple_names_three import (
...
from solver import check_equation_order, solve_linear_equation_from_string
from StructureGraphThree import GraphStructure, Node
    ...
    for i in range(len(topo)):
        for j in range(i + 1, len(topo)):
            if topo[i] in topo[j].edgetolist:
                print(colored(f"{topo[j]} --> {topo[i]}", "red"))
                return False
        for j in range(i):
            if topo[i] in topo[j].edgefromlist:
                print(colored(f"{topo[i]} --> {topo[j]}", "red"))
                return False
    return True
...
def drawStructure(e, d, w0, w1, template=None):
    ...
    if template == "crazy_zootopia":
        G_structure.attachEnglish(
            hierarchical_categorizations=hierarchical_categorizations, subcategories=subcategories
        )
    elif template == "teachers_in_school":
        G_structure.attachEnglish(
            hierarchical_categorizations=hierarchical_categorizationstwo, subcategories=subcategoriestwo
        )
    elif template == "movie_festival_awards":
        G_structure.attachEnglish(
            hierarchical_categorizations=hierarchical_categorizationsthree, subcategories=subcategoriesthree
        )
...
def recursivelyaddpara(
    G_structure, parentnode, parentparameter, end_layer, parentlevelinstanceparameter=None
):
    # helper
    outlist = {}
    outtwo = []
    ...
    if end_layer - parentnode.leveli == 0:
        ...
        for adjacentnode in parentnode.adjacent:
            if adjacentnode.leveli == end_layer:
                ...
                if parentlevelinstanceparameter == None:
                    name = f"{adjacentnode.name} in {parentnode.name}"
                else:
                    name = f"{adjacentnode.name} per {parentlevelinstanceparameter.name}"
                newinstanceparameter = InstanceParameter(name)
                if (
                    newinstanceparameter not in parentparameter.edgefromlist
                ):  # in the parent parameter case, we might have already added
        ...
        for adjacentnode in parentnode.adjacent:
            if adjacentnode.leveli > parentnode.leveli:
                ...
                newinstanceparameter = InstanceParameter(name)
                if G_structure.d == 2:
                    name = f"{G_structure.layer_names[end_layer]} in {adjacentnode.name}"
                    newabstractparameter = AbstractParameter(name)
                else:
                    name = "{} per {}".format(
                        G_structure.layers[end_layer][0].name, newinstanceparameter.name
                    )
                    newabstractparameter = AbstractParameterSpecial(name, newinstanceparameter)
                abvalidtwo, outlistt, outtwoot = recursivelyaddpara(
                    G_structure, adjacentnode, newabstractparameter, end_layer, newinstanceparameter
                )
                        ...
                        if key in outlist.keys():
                            ...
                            for parameter in outlistt[key]:  # merging
                                if parameter in outlist[key]:
                                    ...
                                    parameter2.edgetolist = combine_unique(
                                        parameter2.edgetolist, parameter.edgetolist
                                    )
                                    parameter2.edgefromlist = combine_unique(
...
def all_abstract_parameters(G_structure):
    ...
    for lengthleap in range(G_structure.d - 1, 0, -1):
        for node in G_structure.layers[0]:
            ...
            assert len(node.adjacent) != 0, f"node {node.name} has no adjacent nodes."
            name = f"{G_structure.layer_names[lengthleap]} in {node.name}"
            newabstractparameter = AbstractParameter(name)
            if (lengthleap) not in outlist.keys():
                outlist[lengthleap] = [newabstractparameter]
            else:
                outlist[lengthleap] += [newabstractparameter]
            abvalid, outlistt, outtwoot = recursivelyaddpara(
                G_structure, node, newabstractparameter, lengthleap
            )
            if abvalid:
                if len(newabstractparameter.edgefromlist) == 0:
...
def drawNecessary2(G_necessary, template=None):
    ...
    topo: list[ParameterNode] = []
    cuewordss = {
        "crazy_zootopia": "animal children",
        "teachers_in_school": "teachers from all schools",
        "movie_festival_awards": "movie nominations",
    }
        ...
        if len(intersection(nextone, nexttwo)) == 0:
            ...
            if isinstance(parameter0, AbstractParameter):
                if isinstance(parameter0, AbstractParameterSpecial):
                    print(
                        colored(
                            "parameter0.isonamicparameter in {}".format(
                                (parameter0.isonamicinstanceparameter) in topo
                            ),
                            "red",
                        )
                    )
...
def generate_sentence(G_necessary, item, number_range=23, specialparameter=None):
    ...
    text = f"{standard_name(item.name, sentence_begins=True)} equals"
    # pool←{b∈Gd:∃(b→a)∈Gd}.
    pool = []
    for item2 in item.edgefromlist:
        pool.append(item2)
    ...
    if len(pool) == 1:
        ...
        text += f" {standard_name(pool[0].name)}"
    # else if |pool| = |{b, c}| = 2 then
    elif len(pool) == 2:
        # str ← str + “ the sum of [b] and [c]” or “ the difference of [b] and [c]” each w.p. 0.5.
        if random.random() < 0.5:
            text += f" the sum of {standard_name(pool[0].name)} and {standard_name(pool[1].name)}"
            item.notation = "+"
        else:
            text += " the difference between {} and {}".format(
                standard_name(pool[0].name), standard_name(pool[1].name)
            )
            item.notation = "-"
    # else
    elif len(pool) > 2:
        ...
        text += " the sum of "
        for item in pool[:-1]:
            text += f"{standard_name(item.name)}, "
        text += f"and {standard_name(pool[-1].name)}"
    else:
    ...
    if item != specialparameter:
        ...
        if item.name.split(" ")[0] == "average":
            text = f"{standard_name(item.name, sentence_begins=True)} is greater than 0"
        else:
            text = "{} exists, and its number is greater than 0".format(
                standard_name(item.name, sentence_begins=True)
            )
        return text
...
def generate_description_structure(G_structure):
    ...
    for l in range(G_structure.d):
        text += f"Layer {l} has category {G_structure.layer_names[l]}"
        text += "; "
        text += f"On layer, the specific types of {G_structure.layer_names[l]} are "
        text += ", ".join([item.name for item in G_structure.layers[l]])
        text += ";\n"

    for l in range(G_structure.d - 1):
        text += f"For types on layer {l}, we define the connections as follows: "
        for item in G_structure.layers[l]:
            text += "type {} has the following types in the next layer {}".format(
                item, G_structure.layer_names[l + 1]
            )
            sub_types = []
...
def problem_in_text(G_necessary, number_range=23, specialparameter=None):
    ...
    for instanceparameter in G_necessary.instanceparameters:
        newsentence = generate_sentence(
            G_necessary, instanceparameter, number_range=number_range, specialparameter=specialparameter
        )
...
def question_in_text(specialparameter):
    ...
    if "average" in itemname or "total" in itemname:
        text = f"What is the {itemname}?"
    else:
        category = itemname.split(" in ")[-1]
        object_subcategory = itemname.split(" in ")[0]
        text = f"How many {object_subcategory} does {category} have?"
    return text
...
def solution_in_text(G_necessary, topo, specialparameter):
        ...
        if isinstance(item, InstanceParameter):
            ...
            if len(pool) == 0:  # initialized
                if item == specialparameter:
                    ...
                    text1 += f"so {new_char} = {item.reversibletext}"
                ...
                if not rngin:
                    text1 += f"so {new_char} = {pool[0].variable} = {item.reversibletext}"
                else:
                    text1 += f"{new_char} = {pool[0].variable} = {item.reversibletext}"
                ...
                if item.notation == "+":
                    text1 += "{} + {} = {} + {}".format(
                        pool[0].variable, pool[1].variable, pool[0].reversibletext, pool[1].reversibletext
                    )
                    item.value = pool[0].value + pool[1].value
                    item.reversibletext = sympify(f"{pool[0].reversibletext} + {pool[1].reversibletext}")
                    text1 += f" = {item.reversibletext}"
                elif item.notation == "-":
                    text1 += "{} - {} = ({}) - ({})".format(
                        pool[0].variable, pool[1].variable, pool[0].reversibletext, pool[1].reversibletext
                    )
                ...
                for idx in range(1, len(pool)):  # running multiple factors
                    if idx == 1:
                        new_char = random.choice(ch)[0]
                        ch.remove(new_char)
                        text1 += f"{new_char} = {pool[idx - 1].variable} + {pool[idx].variable}"
                        item.variable = new_char
                        item.value = pool[idx - 1].value + pool[idx].value
                        item.reversibletext = sympify(
                            f"{pool[idx - 1].reversibletext} + {pool[idx].reversibletext}"
                        )
                        text1 += " = {} + {} = {}; ".format(
                            pool[idx - 1].reversibletext, pool[idx].reversibletext, item.reversibletext
                        )
                    else:
                        new_char = random.choice(ch)[0]
                        ch.remove(new_char)
                        if idx == len(pool) - 1 and not rngin:
                            text1 += "so "
                        text1 += f"{new_char} = {item.variable} + {pool[idx].variable}"
                        item.variable = new_char
                        if idx == len(pool) - 1:
                            text1 += " = {} + {} = {}".format(
                                item.reversibletext,
                                pool[idx].reversibletext,
                                sympify(f"{item.reversibletext} + {pool[idx].reversibletext}"),
                            )
                        else:
                            text1 += " = {} + {} = {}; ".format(
                                item.reversibletext,
                        ...
                        item.reversibletext = sympify(f"{item.reversibletext} + {pool[idx].reversibletext}")
            ...
            if G_necessary.rng in item.edgefromlist and item.rngnot != None:
                ...
                if item.rngnot == "+":
                    text1 += "{} = {} + {} = {} + {} = {}. ".format(
                        ...
                        item.rngv,
                        item.reversibletext,
                        sympify(f"{item.rngv} + {item.reversibletext}"),
                    )
                    item.value = item.rngv + item.value
                    item.reversibletext = sympify(f"{item.rngv} + {item.reversibletext}")
                    item.variable = new_char
                elif item.rngnot == "*":
                    text1 += "{} = {} * {} = ({}) * ({}) = {}. ".format(
                        new_char,
                ...
                solution_text += f"Define {item.name} as {item.variable}; "
                solution_text += text1
            else:
                solution_text += f"Define {item.name} as {item.variable}; "
                solution_text += text1 + ". "
            ...
            for pairr in list_pair:
                ...
                text1 += "{} = {} * {} = ({}) * ({}) = {}; ".format(
                    ...
                    inpara.variable,
                    abpara.variable,
                    inpara.reversibletext,
                    abpara.reversibletext,
                    expand(sympify(f"({inpara.reversibletext}) * ({abpara.reversibletext})")),
                )
                accum_pair.append((new_char, inpara.value * abpara.value))
                accum_rtextpair.append(
                    (
                        new_char,
                        expand(sympify(f"({inpara.reversibletext}) * ({abpara.reversibletext})")),
                    )
            ...
            for idx in range(1, len(accum_pair)):
                ...
                text1 += "{} = {} + {} = {} + {} = {}; ".format(
                    ...
                    item.reversibletext,
                    reversibletx,
                    sympify(f"{item.reversibletext} + {reversibletx}"),
                )
                item.value = item.value + val
                item.reversibletext = sympify(f"{item.reversibletext} + {reversibletx}")
                item.variable = new_char
            ...
            if len(list_factor) == 1:
                ...
                text1 += "so "
                text1 += "{} = {} = {}; ".format(
                    new_char, list_factor[0].variable, list_factor[0].reversibletext
                )
                ...
                for i in range(len(list_factor)):
                    ...
                    text1 += "{} = {} + {} = {} + {} = {}; ".format(
                        ...
                        item2.variable,
                        item.reversibletext,
                        item2.reversibletext,
                        sympify(f"{item.reversibletext} + {item2.reversibletext}"),
                    )
                    item.value = item.value + item2.value
                    item.reversibletext = sympify(f"{item.reversibletext} + {item2.reversibletext}")
                    item.variable = new_char
            solution_text += f"Define {item.name} as {item.variable}; "
            text1 = text1[:-2] + ". "
            solution_text += text1
    ...
    text2 = ""
    text2 += f"We know {topo[-1].variable} = {topo[-1].value}, "
    equat = f"{topo[-1].reversibletext} = {topo[-1].value}"
    text2 += f"so we have {equat} and we solve it through the following steps: "
    ...
    if outputorder == "first-order":
        ...
        solution_text += text2
        solution_text += ". "
        solution_text += f"Answer: {solution}."
...
def dummynetwokinginstanceparam(
    ...
    print(
        colored(
            "misslength {} total_add_statements {} added_abstract_num {}".format(
                misslength, total_add_statements, added_abstract_num
            ),
            "cyan",
        )
    )
    ...
    subcategoriess = random.sample(
        sorted(dummysubcategoriess[template][upperlevelname].keys()), (added_abstract_num // 20) + 1
    )
    listtop = [
        x
        for subcategory in subcategoriess
        for x in dummysubcategoriess[template][upperlevelname][subcategory]
    ]
    ...
    for nametop in listtop:
        ...
        if abstractparamcount >= added_abstract_num:
            break
    print(colored(f"number of keys in layers {len(layers.keys())}", "cyan"))
    ...
    for i, abstractparam in enumerate(layers.keys()):
        ...
        for namebot in listbot:
            layers[abstractparam].append(f"{namebot.lower()} in {nametop}")
            instance_count += 1
            if depth == 2:
                if instance_count >= total_add_statements // 2:
                    print(colored(f"instance count {instance_count}", "cyan"))
                    break
            else:
        ...
        if depth == 2:
            ...
            for item in layers[abstractparam]:
                addedlist.append(
                    f"{list(dummysubcategoriess[template][namethird].values())[0][0]} per {item}"
                )
    ...
    for abstractparam in layers.keys():
        abstract_parameter_node = AbstractParameter(f"{abstractparam[0]} in {abstractparam[1]}")
    ...
    print(
        colored(
            "abstract count {} instance count {}".format(
                len(list_abstract_parameters), len(all_instance_parameters)
            ),
            "cyan",
        )
    )
    ...
    print(
        colored(
            "Statistics - mean {} max {} min {}".format(
                np.mean(listfromna), np.max(listfromna), np.min(listfromna)
            ),
            "cyan",
        )
    )
    print(
        colored(
            "number of actual statements {} misslength {} havelength {}".format(
                len(additional_sentences), misslength, havelength
            ),
            "cyan",
        )
    )
    ...
    for item in core_graph.abstractparameters:
        checkingsum2 += len(item.edgefromlist)
    print(colored(f"checkingsum {checkingsum} checkingsum2 {checkingsum2}", "cyan"))
...
def generate_problem_text(
    ...
    for i in range(6):
        total_text += (
            "Problem: " + problem_text + " Question: " + question_text + " Solution: " + solution_text + "\n"
        )
    total_text += "Problem: " + problem_text + " Question: " + question_text + " Solution: "
    length = len(
        tokenizer.encode(total_text)
    )  # although there is some special prompt tokens not taken in consideration, but it is fine for now
    misslength = target_length - length
    print(colored(f"target length {target_length} length {length} misslength {misslength}", "yellow"))
    ...
    if target_length != None and misslength > 0:
        ...
        list_statements += dummynetwokinginstanceparam(
            misslength,
            G_necessary,
            number_range,
            template,
            ...
            offset,
            topo,
            lengthmessageone=lengthmessageone,
        )
...
def drawAllEquan(
    ...
    G_structure = drawStructure(
        e, d, w0, w1, template=template
    )  # e is number of edges, d is number of layers, w0 is the minimum number of nodes per layer, w1 is the maximum number of nodes per layer
    if verbose:
        G_structure.draw()
    ...
    problem_segments = problem_in_text(
        G_necessary=G_necessary, number_range=number_range, specialparameter=parameter
    )
    ...
    if strictline != -1:
        if numberopscalculated > strictline:
            raise ValueError(
                f"Number of operations {numberopscalculated} is greather than the strict requirement"
            )
...
if __name__ == "__main__":
    ...
    print(colored(f"Problem: {problem}", "yellow"))
    print(colored(f"Question: {question}", "cyan"))
    print(colored(f"Solution: {solution}", "green"))
