...
class DependencyGraph:
        ...
        for item in listab:
            if item not in self.abstractparameters:
                if len(item.edgefromlist) != 0:
                    ...
                    print(
                        "item {} edgefromlist {} edgetolist {}".format(
                            item, item.edgefromlist, item.edgetolist
                        )
                    )
                    exit(0)
                    continue
                ...
                item2.edgefromlist = item.edgefromlist + [
                    k for k in item2.edgefromlist if k not in item.edgefromlist
                ]
    ...
    def drawmess(self, core_graph):
        ...
        for item in self.instanceparameters:
            if item in coreparams:
                continue
            name = f"{item.name}({len(item.edgefromlist)})"
            if item.coretoflag == 0:
                node = pydot.Node(
                    name, shape="rectangle", fillcolor="lightblue", color="none", style="filled"
                )
            else:
                node = pydot.Node(
                    name, shape="rectangle", fillcolor="lightgreen", color="none", style="filled"
                )
            graph.add_node(node)
            allnodelist.append(node)
        ...
        for item in self.abstractparameters:
            ...
            for item2 in item.edgefromlist:
                if item2 in coreparams:
                    continue
                name = f"{item2.name}({len(item2.edgefromlist)})"
        ...
        for item in self.instanceparameters:
            for item2 in item.edgefromlist:
                if item2 in coreparams:
                    continue
                name = (
                    f"{item2.name}({len(item2.edgefromlist)})"
                    if not isinstance(item2, AbstractParameter)
                    else item2.name
                )
                graph.add_edge(pydot.Edge(name, f"{item.name}({len(item.edgefromlist)})", color="black"))
    ...
    def draw1(self):
        ...
        for item in self.abstractparameters:
            node = pydot.Node(
                item.name + " (a)", shape="ellipse", fillcolor="lightblue", color="none", style="filled"
            )
            graph.add_node(node)
            allnodelist.append(node)
        ...
        for item in self.instanceparameters:
            node = pydot.Node(
                item.name + " (i)", shape="rectangle", fillcolor="lightblue", color="none", style="filled"
            )
            graph.add_node(node)
            allnodelist.append(node)
        ...
        for item in self.abstractparameters:
            ...
            for item2 in item.edgefromlist:
                childrenname = (
                    item2.name + " (a)" if isinstance(item2, AbstractParameter) else item2.name + " (i)"
                )
    ...
    def visualization_topo(self, topo: list[ParameterNode]):
        ...
        graph.set_size(f"{len(allnodelist) * 5},{len(allnodelist) * 5}")
    ...
    def draw4(self, topo: list[ParameterNode]):
        ...
        for item in self.abstractparameters:
            if item in topo:
                if topo.index(item) == len(topo) - 1:
                    node = pydot.Node(
                        item.name + " (a)", shape="ellipse", fillcolor="cyan", color="none", style="filled"
                    )
                else:
                    node = pydot.Node(
                        item.name + " (a)",
                ...
                node = pydot.Node(
                    item.name + " (a)", shape="ellipse", fillcolor="purple", color="none", style="filled"
                )
            graph.add_node(node)
            allnodelist.append(node)
        ...
        for item in self.instanceparameters:
            if item in topo:
                if topo.index(item) == len(topo) - 1:
                    node = pydot.Node(
                        item.name + " (i)", shape="rectangle", fillcolor="cyan", color="none", style="filled"
                    )
                else:
                    node = pydot.Node(
                        item.name + " (i)",
                ...
                node = pydot.Node(
                    item.name + " (i)", shape="rectangle", fillcolor="purple", color="none", style="filled"
                )
            graph.add_node(node)
            allnodelist.append(node)
