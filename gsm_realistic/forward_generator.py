...
def solution_in_text(G_necessary, topo, mod: int = -1):
    ...
    for item in topo:
        ...
        if isinstance(item, InstanceParameter):
            ...
            if len(pool) == 0:  # initialized
                ...
                text1 += f"so {new_char} = {item.value}"
                ...
                if not rngin:
                    text1 += f"so {new_char} = {pool[0].variable} = {item.value}"
                else:
                    text1 += f"{new_char} = {pool[0].variable} = {item.value}"
                ...
                if item.notation == "+":
                    text1 += "{} + {} = {} + {}".format(
                        pool[0].variable, pool[1].variable, pool[0].value, pool[1].value
                    )
                ...
                for idx in range(1, len(pool)):  # running multiple factors
                    if idx == 1:
                        ...
                        if mod > 0:
                            item.value = item.value % mod
                        text1 += f" = {pool[idx - 1].value} + {pool[idx].value} = {item.value}; "
                    else:
                        new_char = random.choice(ch)[0]
                        ch.remove(new_char)
                        ...
                        if idx == len(pool) - 1:
                            if mod > 0:
                                text1 += " = {} + {} = {}".format(
                                    item.value, pool[idx].value, (item.value + pool[idx].value) % mod
                                )
                            else:
                                text1 += " = {} + {} = {}".format(
                                    item.value, pool[idx].value, item.value + pool[idx].value
                                )
                        else:
                            if mod > 0:
                                text1 += " = {} + {} = {}; ".format(
                ...
                if item.rngnot == "+":
                    if mod > 0:
                        text1 += "{} = {} + {} = {} + {} = {}. ".format(
                            ...
                            item.rngv,
                            item.value,
                            (item.rngv + item.value) % mod,
                        )
                        item.value = (item.rngv + item.value) % mod
                    else:
                        text1 += "{} = {} + {} = {} + {} = {}. ".format(
                            new_char, item.rngv, item.variable, item.rngv, item.value, item.rngv + item.value
                        )
            ...
            for idx in range(1, len(accum_pair)):
                ...
                if mod > 0:
                    text1 += "{} = {} + {} = {} + {} = {}; ".format(
                        new_char, item.variable, cha, item.value, val, (item.value + val) % mod
                    )
                    item.value = (item.value + val) % mod
                else:
                    text1 += "{} = {} + {} = {} + {} = {}; ".format(
                        new_char, item.variable, cha, item.value, val, item.value + val
                    )
            ...
            if len(list_factor) == 1:
                ...
                ch.remove(new_char)
                text1 += "so "
                text1 += f"{new_char} = {list_factor[0].variable} = {list_factor[0].value}; "
    ...
    solution_text += f"Answer: {topo[-1].value}."
...
def dummylistinstanceparam(missingstatements):
    ...
    total_combination = [f"{inner} in {outer}" for outer, inner in total_combination]
...
def drawAll(
    ...
    if oplist is not None:
        print(f"number of operations {num_op(topo)} not in oplist {oplist}")
