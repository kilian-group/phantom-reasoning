...
Example usage:
```bash
python plot_reasoning.py -od out --filter_by_depth 20 --model_list meta-llama/llama-3.3-70b-instruct
```
"""
...
# NOTE: uncomment the following to use Times New Roman for the font
# plt.rcParams.update(
#     {
#         "font.family": "serif",
...
parser.add_argument(
    ...
    type=int,
    default=20,
    help="When there are splits with multiple depths in a directory, "
    "Use this argument to specify the depth to plot accuracies for",
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=int,
    default=42,
    help="To hard-code a model color, modify the plotting_utils.py file in this directory. "
    "Otherwise, the model color will be randomly assigned using this seed",
)
...
def get_color(model, method, by_model=True):
    """Get color for a line or handle

    NOTE: models and methods with hard-coded colors are listed in plotting_utils.py
    To update the colors, modify the plotting_utils.py file in this directory.
    Otherwise, the color will be randomly assigned using the seed specified in the parser.

    Args:
        model (str): model name
        method (str): method name
        by_model (bool): if true, pick color by model name;
            if false, pick color by method name
    Returns:
        color (str): color
        ...
        for model in model_list:
            ...
            model_handles.append(
                lines.Line2D(
                    [0],
                    [0],
                    color=get_color(model, method, by_model=True),
                    label=key,
                    linewidth=1,
                )
            )
