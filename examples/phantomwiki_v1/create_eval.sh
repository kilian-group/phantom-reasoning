...
cat << EOT > slurm_scripts/${JOB_NAME}.slurm
#!/bin/bash
...
chmod +x slurm_scripts/${JOB_NAME}.slurm
...
if [[ "$LAUNCH" =~ ^[Yy]$ ]]; then
    ...
    sbatch slurm_scripts/${JOB_NAME}.slurm $OUTPUT_DIR
else
    echo "Slurm script generated. To launch the script, run:"
    echo "sbatch slurm_scripts/${JOB_NAME}.slurm OUTPUT_DIR --flag flag_value"
    echo ""
    echo "To specify environment variables such as DATASET=kilian-group/phantom-wiki-v1, run:"
    echo "sbatch --export=ALL,DATASET='kilian-group/phantom-wiki-v1' slurm_scripts/${JOB_NAME}.slurm OUTPUT_DIR --flag flag_value"
fi
