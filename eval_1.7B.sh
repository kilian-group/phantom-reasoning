#!/usr/bin/env bash
#SBATCH --job-name=evalQwen3-1.7B
#SBATCH --output=logs/evalQwen3-1.7B-%j.out
#SBATCH --error=logs/evalQwen3-1.7B-%j.err
#SBATCH -p kilian
#SBATCH -N 1
#SBATCH -n 8
...
until curl -s http://localhost:8000/v1/models | grep -q Qwen3-1.7B; do
    echo "Still waiting..."
    sleep 5
done
echo "vLLM is ready."
