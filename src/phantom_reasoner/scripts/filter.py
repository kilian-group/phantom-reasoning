...
def filter_traces(prediction_dir, metric, threshold=1):
    ...
    )  # All out repos contain "preds" by default in our phantom-eval implementation

    # Get all method directories, filtering out hidden files/directories
    method_dirs = [
        os.path.join(prediction_dir, i) for i in os.listdir(prediction_dir) if not i.startswith(".")
    ]
...
def save_filtered_traces(filtered_traces, output_dir, metric, threshold):
    ...
    full_output_dir = os.path.join(
        os.path.join(base_dir, filter_dir_name), "preds"
    )  # Save to preds to stay consistent with norms
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Filter prediction traces based on quality metrics.")
