...
def update_gold_with_aliases(row: dict, aliases: dict) -> set[str]:
    """
    Add aliases to the gold answers.
    Args:
        row: A row of the gold dataframe.
...
def score_pred(row: dict) -> dict:
    """
    Compute the following metrics for a prediction:
    - em: exact match
    - f1: f1 score
    - prec: precision
    - recall: recall
    ...
    Args:
        row: A row of the prediction dataframe.
    Returns:
...
def get_preds(output_dir: str, data_dir: str, dataset: str, split: str, method: str) -> pd.DataFrame:
    ...
    df_gold["answer"] = df_gold.apply(lambda row: update_gold_with_aliases(row, aliases), axis=1)
