#!/usr/bin/env bash
#SBATCH --job-name=evalQwen3-32b
#SBATCH --output=logs/evalQwen3-32b-%j.out
#SBATCH --error=logs/evalQwen3-32b-%j.err
#SBATCH -p kilian
#SBATCH -N 1
#SBATCH -n 8
...
until curl -s http://localhost:8000/v1/models | grep -q Qwen3-32B; do
    echo "Still waiting..."
    sleep 5
done
echo "vLLM is ready."
...
# 启动评估
CUDA_VISIBLE_DEVICES=0,1 python -m phantom_eval \
  --method cot \
  --server vllm \
  ...
  --from_local \
  --inf_vllm_tensor_parallel_size 2 \
  -od ./evals/Qwen3-32B
