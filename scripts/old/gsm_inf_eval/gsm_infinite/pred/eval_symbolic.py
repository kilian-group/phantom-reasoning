...
def get_prompt(reply, value):
    ...
    return f"""
    Carefully read the input text below and extract the variable names that are
    mentioned as being equal to {value}. If multiple variables are listed,
    separate them with commas.

    If value {value} is not mentioned, you can safely assume that they are
    all equal to {value}. If there are no such variables, just output None.

    Only output the variable names, not the values or any other text.

    Examples:
    1. input: "V0, V3, V4", output: "V0, V3, V4"
    2. input: "Variable V2 is equal to {value} in the assignment statement
    'assign V2 = V1 - 1.'.", output: "V2"
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--dataset-name', type=str, help="The name of the dataset
    # for organizing the folders")
    ...
    pipeline = NoRAGPipeline(model_handler=model_handler, temperature=0.0, max_tokens=2048)
    ...
    try:
        dir_name = "datasets"
        with open(f"{dir_name}/symbolic-{args.save_name}_{str(length)}") as f:
        ...
        for sample_num in sample_nums:
            ...
            with open(
                f"results/result_symbolic_{args.save_name}{file_sample_suffix}.txt",
                "a+",
            ) as file:
                for op in sorted_keys:
                    ...
                    acc = format(correctnum / count_dict[op], ".4f").rstrip("0").rstrip(".")
                    file.write(f"length: {length}, op: {op}, acc: {acc}\n")
