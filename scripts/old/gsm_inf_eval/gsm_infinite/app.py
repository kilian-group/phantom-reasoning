...
selected_length = st.sidebar.selectbox(
    ...
    options=lengths,
    key="length_selector",
    index=lengths.index(0) if 0 in lengths else 0,
)
...
if has_subset_info:
    ...
    selected_modes = st.sidebar.multiselect(
        "Select Modes", options=mode_options, default=["all"], key="mode_selector"
    )
...
selected_palette = st.sidebar.selectbox(
    "Color Palette", options=list(color_palettes.keys()), key="palette_selector"
)
...
if st.sidebar.button("Add Series to Plot"):
    ...
    if has_subset_info and selected_templates and selected_modes:
        ...
        template_str = (
            ...
            if len(selected_templates) < len(available_templates)
            else "all templates"
        )
        mode_str = ", ".join(selected_modes) if len(selected_modes) < len(available_modes) else "all modes"
...
if st.session_state.selected_series:
    ...
    if show_average and len(st.session_state.selected_series) > 1:
        ...
        all_n_values = sorted({n for series in st.session_state.selected_series for n in series["x"]})
    ...
    detail_series = next(
        (s for s in st.session_state.selected_series if s["label"] == selected_detail_series),
        None,
    )

    if detail_series:
        # Create a DataFrame with the detailed data
        detail_df = pd.DataFrame({"Op": detail_series["x"], "Accuracy": detail_series["y"]})
